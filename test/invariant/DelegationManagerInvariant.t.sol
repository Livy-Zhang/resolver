...
contract DelegationManagerHandler is Test {
    ...
    function delegate(uint256 delegatorSeed, uint256 resolverSeed, uint256 amountSeed) external {
        ...
        uint256 amount = bound(amountSeed, 100 ether, 1000 ether);

        vm.startPrank(delegator);

        stakingToken.approve(address(delegationManager), amount);
    ...
    function undelegate(uint256 delegatorSeed, uint256 amountSeed) external {
        address delegator = delegators[bound(delegatorSeed, 0, delegators.length - 1)];

        (uint256 delegatedAmount, , address resolver) = delegationManager.delegations(delegator);

        if (delegatedAmount == 0) return;

        uint256 amount = bound(amountSeed, 1, delegatedAmount);
    ...
    function redelegate(uint256 delegatorSeed, uint256 resolverSeed) external {
        address delegator = delegators[bound(delegatorSeed, 0, delegators.length - 1)];

        address newResolver = resolvers[bound(resolverSeed, 0, resolvers.length - 1)];

        (uint256 amount, , address oldResolver) = delegationManager.delegations(delegator);
...
contract DelegationManagerInvariantTest is Test {
    ...
    function setUp() public {
        ...
        stakingToken = new MockERC20(owner, 100000000 ether);

        rewardToken = new MockERC20(owner, 100000000 ether);
        ...
        handler = new DelegationManagerHandler(
            delegationManager,
            stakingManager,
            stakingToken,
            owner
    ...
    function invariant_totalDelegatedAccounting() public {
        ...
        for (uint256 i; i < handler.delegatorsLength(); i++) {
            (uint256 amount, , ) = delegationManager.delegations(handler.delegators(i));

            total += amount;
        }

        assertEq(total, delegationManager.totalDelegated());
    }
    ...
    function invariant_resolverAccounting() public {
        ...
        for (uint256 i; i < handler.resolversLength(); i++) {
            address resolver = handler.resolvers(i);

            uint256 amount = delegationManager.delegationTo(resolver);

            assertEq(amount, handler.ghostResolverDelegated(resolver));
    ...
    function invariant_validDelegation() public {
        for (uint256 i; i < handler.delegatorsLength(); i++) {
            (uint256 amount, , address resolver) = delegationManager.delegations(
